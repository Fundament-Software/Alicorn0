...
local function let_impl(syntax, env)
	...
	if format.is_symbol(name) then
		...
		env = env:bind_local(terms.binding.tuple_elim(terms.symbol_array(table.unpack(names)), expr, syntax.start_anchor, syntax.end_anchor))
	end
...
local ascribed_name = metalanguage.reducer(
	...
	function(syntax, env, prev, names)
		...
		local ok, prev_binding = env:get(create_symbol(prev_name, nil_anchor, nil_anchor))
		if not ok then
			error "#prev should always be bound, was just added"
		end
		---@cast prev_binding -string
		env = env:bind_local(terms.binding.tuple_elim(names, prev_binding, syntax.start_anchor, syntax.end_anchor))
...
local function make_host_func_syntax(effectful)
	...
	local function host_func_type_impl(syntax, env)
		...
		if params_single then
			env = env:bind_local(terms.binding.let(params_names, arg, start_anchor, end_anchor))
		else
			env = env:bind_local(terms.binding.tuple_elim(params_names, arg, start_anchor, end_anchor))
		end
...
local function forall_impl(syntax, env)
	...
	local ok, arg = env:get(create_symbol(inner_name, nil_anchor, nil_anchor))
	if not ok then
		error("wtf")
	end
	---@cast arg -string
...
local function lambda_impl(syntax, env)
	...
	local _, arg = inner_env:get(create_symbol(inner_name, nil_anchor, nil_anchor))
	inner_env = inner_env:bind_local(terms.binding.tuple_elim(names, arg, syntax.start_anchor, syntax.end_anchor))
	local ok, expr, env = tail:match(
