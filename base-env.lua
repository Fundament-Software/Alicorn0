...
local function lit_term(val, typ)
	return anchored_inferrable_term(
		...
		unanchored_inferrable_term.typed(typ, usage_array(), terms.typed_term.literal(val))
	)
end
...
local function mk_impl(syntax, env)
	...
	return ok, anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.enum_cons(name.str, tuple)), env
end
...
local function switch_impl(syntax, env)
	...
	return true,
		anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.enum_case(subj, variants)),
		env
end
...
local function record_build(syntax, env)
	...
	return true, anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.record_cons(map)), env
end
...
local tuple_desc_of_ascribed_names = metalanguage.reducer(
	...
	function(syntax, env)
		local function build_type_term(start_anchor, args)
			return anchored_inferrable_term(start_anchor, unanchored_inferrable_term.tuple_type(args))
		end

		local inf_array = gen.declare_array(anchored_inferrable_term)
		---@param start_anchor Anchor
		---@vararg anchored_inferrable
		---@return anchored_inferrable
		local function tup_cons(start_anchor, ...)
			return anchored_inferrable_term(start_anchor, unanchored_inferrable_term.tuple_cons(inf_array(...)))
		end
		---@param start_anchor Anchor
		---@param ... anchored_inferrable[]
		---@return anchored_inferrable
		local function cons(start_anchor, ...)
			return anchored_inferrable_term(
				start_anchor,
				unanchored_inferrable_term.enum_cons(terms.DescCons.cons, tup_cons(start_anchor, ...))
			)
		end
		---@type anchored_inferrable
		local empty = anchored_inferrable_term(
			syntax.start_anchor,
			unanchored_inferrable_term.enum_cons(terms.DescCons.empty, tup_cons(syntax.start_anchor))
		)
		---@type string[]
		...
		local ok, thread = syntax:match({
			metalanguage.list_many_fold(function(_, vals, thread)
				return true, thread
			end, function(thread, start_anchor)
				return ascribed_name(function(_, name, type_val, type_env)
					local names = thread.names:copy()
					names:append(name)
					local newthread = {
						names = names,
						args = cons(start_anchor, thread.args, type_val),
						env = type_env,
					}
					return true, { name = name, type = type_val }, newthread
				end, thread.env, build_type_term(start_anchor, thread.args), thread.names)
			end, {
				names = names,
				args = empty,
				env = env,
...
local tuple_of_ascribed_names = metalanguage.reducer(
	...
	function(syntax, env)
		...
		thread.args = anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.tuple_type(thread.args))
		return ok, thread
	end,
...
local host_tuple_of_ascribed_names = metalanguage.reducer(
	...
	function(syntax, env)
		...
		thread.args =
			anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.host_tuple_type(thread.args))
		return ok, thread
	end,
...
local tuple_desc_wrap_ascribed_name = metalanguage.reducer(
	...
	function(syntax, env)
		...
		names = names:copy()
		names:append(name)
		args = cons(syntax.start_anchor, args, type_val)
		env = type_env
...
local host_ascribed_segment_tuple = metalanguage.reducer(function(syntax, env)
	...
	thread.args = anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.host_tuple_type(thread.args))
	return ok, thread
...
local function make_host_func_syntax(effectful)
	...
	local function host_func_type_impl(syntax, env)
		...
		local start_anchor = (tail.start_anchor or syntax.start_anchor)
...
local function the_operative_impl(syntax, env)
	...
	return ok,
		anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.annotated(val, type_inferrable_term)),
		env
end
...
local function into_operative_impl(syntax, env)
	...
	return true,
		anchored_inferrable_term(
			syntax.start_anchor,
			unanchored_inferrable_term.typed(op_type, usage_array(), typed.literal(op_val))
		),
		env
end
...
local enum_variant = metalanguage.reducer(function(syntax, env)
	...
	return true,
		tag.str,
		anchored_inferrable_term(syntax.start_anchor, unanchored_inferrable_term.tuple_type(tail.args)),
		env
end, "enum_variant")
