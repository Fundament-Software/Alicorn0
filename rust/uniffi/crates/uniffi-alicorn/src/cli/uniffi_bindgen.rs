...
use crate::AlicornBindingGenerator;
use uniffi::{
...
use {
    uniffi::CargoMetadataConfigSupplier,
    uniffi_bindgen::BindgenCrateConfigSupplier as UpstreamBindgenCrateConfigSupplier,
};
...
impl TryFrom<&str> for TargetLanguage {
    ...
    fn try_from(value: &str) -> Result<Self> {
        Ok(match value.to_ascii_lowercase().as_str() {
            "alicorn" | "alc" => TargetLanguage::Alicorn,
...
fn config_supplier(metadata_no_deps: bool) -> Result<impl UpstreamBindgenCrateConfigSupplier> {
    #[cfg(feature = "cargo-metadata")]
    return {
        ...
        Ok(CargoMetadataConfigSupplier::from(metadata))
    };
    #[cfg(any(rust_analyzer, not(feature = "cargo-metadata")))]
    Ok(EmptyCrateConfigSupplier)
}
...
fn gen_library_mode(
    ...
    for language in languages {
        ...
        // for Swift and Alicorn where static libs are often used to extract the metadata.
        if !matches!(language, TargetLanguage::Alicorn | TargetLanguage::Swift)
            && !uniffi_bindgen::is_cdylib(library_path)
        {
            anyhow::bail!(
                "Generating bindings for {language} requires a cdylib, but {library_path} was given"
            );
        }

        // Type-bounds on trait implementations makes selecting between languages a bit tedious.
        match language {
            TargetLanguage::Alicorn => generate_bindings(
                library_path,
                crate_name.clone(),
                &AlicornBindingGenerator,
                &config_supplier,
                cfo,
                out_dir,
                fmt,
