...
local function unformat_list(ast, prev_line, indentation)
	...
	while ast.anchor.line > prev_line do
	...
	elseif ast.kind == "comment" then
		...
		local multiline_comment = not (ast.anchor.line == ast.endpos.line)
	...
	elseif ast.kind == "string" then
		...
		local multiline_string = not (ast.anchor.line == ast.endpos.line)
...
local function unformat(ast)
	...
	for _, v in ipairs(ast.elements) do
		acc = acc .. "\n" .. unformat_list(v, v.anchor.line, 0) .. ""
	end
