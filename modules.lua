...
local module_type = {
  kind = {
    ...
    type_name = function() return "module" end,
...
local function new_mod(values)
  ...
  if self.values == nil then error "tried to new_mod something that becomes nil" end
...
local function use_mod_op_impl(syntax, env)
  ...
  if not ok then return ok, modval end
...
local function get_op_impl(syntax, env)
  local ok, modval, env, name = syntax:match(
    {
      metalang.listmatch(metalang.accept_handler, {evaluator.evaluates(metalang.accept_handler, env), metalang.issymbol(metalang.accept_handler)})
    },
    metalang.failure_handler,
    nil
  )
...
local function build_mod(tab)
  local tmp = {}
  for k, v in pairs(tab) do tmp[k] = {kind = "reusable", val = v} end
  local val = {values = treemap.build(tmp)}
  if val.values == nil then error "tried to build a module that went to nil" end
